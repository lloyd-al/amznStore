            ...
            public async Task OnActionExecutionAsync(ActionExecutingContext context, ActionExecutionDelegate next)
            {
                ...
                if (rolesString != null)
                {
                    ...
                    else
                    {
                        ...
                        {
                            await context.HttpContext.Response.WriteAsync(JsonSerializer.Serialize(new { statuscode = HttpStatusCode.Forbidden, message = $"You are not authorized to use this function." }));
