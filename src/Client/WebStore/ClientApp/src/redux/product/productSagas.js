...
export function* getAllProductsAsync(action) {
    try {
        const response = yield amznApiInstance.get(`/Product?PageSize=${action.pageSize}&PageNumber=${action.currentPage}`)
...
export function* getCategoryProductsAsync(action) {
    try {
        const response = yield amznApiInstance.get(`/Product/GetProductsByCategory/${action.categoryId}`)
        const data = yield response.data
        yield put(getProductsSuccess(data));
    } catch (error) {
        yield put(getProductsError(error.message));
